    ...
    for cutoff in range(1,9,1):
        for i in range(1,20000,1):
            if(predictions[i][0]>cutoff/10):
                ...
                y_pr[i]=1
        cost=5*confusion_matrix(ytest, y_pr)[0,1]+1*confusion_matrix(ytest, y_pr)[1,0]
        if(cost1>cost):
            cost1= cost
            cutoff1=cutoff/10
...
k=miss_classification(classifier)
y_pr=[0]*20000
for i in range(1,20000,1):
            if(predictions[i][0]>k):
                y_pr[i]=0
            else:
                y_pr[i]=1
accuracy = accuracy_score(ytest, y_pr)
print(accuracy)
print (confusion_matrix(ytest, y_pr) )
print(confusion_matrix(ytest, y_pr)[0,1])
fpr, tpr, threshold = metrics.roc_curve(ytest, y_pr)
